"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var core_1 = require('@angular/core');
var AlertDirective = (function () {
    function AlertDirective(staticValue) {
        this.staticValue = staticValue;
        this.outputProperty = new core_1.EventEmitter();
    }
    AlertDirective.prototype.onMouseClick = function () {
        var msg = {
            'msg': '지시자입니다.',
            'static': this.staticValue,
            'input': this._inputValue
        };
        this.outputProperty.emit(msg);
    };
    Object.defineProperty(AlertDirective.prototype, "inputValue", {
        set: function (status) {
            this._inputValue = status;
        },
        enumerable: true,
        configurable: true
    });
    __decorate([
        core_1.Output()
    ], AlertDirective.prototype, "outputProperty", void 0);
    __decorate([
        core_1.Input()
    ], AlertDirective.prototype, "inputValue", null);
    AlertDirective = __decorate([
        core_1.Directive({
            selector: '[alertMsg]',
            host: {
                '(click)': 'onMouseClick()'
            }
        }),
        __param(0, core_1.Attribute('staticValue'))
    ], AlertDirective);
    return AlertDirective;
}());
exports.AlertDirective = AlertDirective;
